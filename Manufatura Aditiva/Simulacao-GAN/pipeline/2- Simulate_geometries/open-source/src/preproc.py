...
def preproc(vtk_dir, array_dir, idx_array, idx_file, origin, simmetry, dimension, score):
    angle = [0,45][idx_file]
    units = 9
    size = 16
    if dimension == 2:
        if origin == '-r':
            ...
            if simmetry[1] in ['3','6']:
                ...
                command_vtk = """python src/generate_model_2d_quad.py %s %s %i %i %i %i %s"""%(origin,simmetry,units,size,idx_array,angle,score)
