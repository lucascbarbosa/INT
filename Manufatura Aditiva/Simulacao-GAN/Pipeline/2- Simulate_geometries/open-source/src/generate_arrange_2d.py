...
def create_plane(name,size,location,scale,angles):
    ...
    for i in range(len(array)):
        for j in range(len(array)):
            ...
            if array[i,j] == 0.0:
                ...
                create_plane(name_pixel,pixel_size*1.001,(loc_x,loc_y,0),scale_pixel,degs_pixel)
...
def create_arrange(name,locations_units):
    ...
    # create element
    create_plane("element",1.001*element_size,(0, 0, 0),(1,1,1),(0,0,0))
    ...
    #add handles
    for i in range(2):
        create_plane("arrange {}".format(i+1), arrange_size, locations[i], (1., 0.25, 1.), (0, 0, 0))

    locations = [(float(arrange_size/2.0)+0.0005, 0, 0),(-float(arrange_size/2.0)-0.0005, 0, 0)]
    
    #add side rectangles
#    for i in range(2):
#        create_plane("arrange{}".format(i+3), arrange_size, locations[i], (float(2./(1000*arrange_size)), 3., float(2*(thickness)/arrange_size)), (0, 0, 0))
    
    join(name, "arrange")
    
    #remesh
    bpy.ops.object.modifier_add(type='REMESH')
    bpy.context.object.modifiers["Remesh"].voxel_size = 0.0001
...
if origin == "-g":
    ...
    stls_dir = r"D:/Lucas GAN/Dados/2- 3D_models/stl/GAN/" + simmetry + '/'
else:
    arrays_dir = r"D:/Lucas GAN/Dados/1- Arranged_geometries/Arrays/RTGA/" + simmetry + "/"
    stls_dir = r"D:/Lucas GAN/Dados/2- 3D_models/stl/RTGA/" + dimension + "D/" + simmetry + "/"
...
for array_filename in arrays_filename[idx:idx+1]:
    with open(os.path.join(arrays_dir,array_filename),'r') as f:
        ...
        # export
        filepath = stls_dir+array_dir+'/'+array_filename[mag-1:-4]+"_theta_%d.stl"%theta
